...
#for key in range(43,len(LLL)):    
    ...
    for win in windows:
        ...
        model = auto_arima(DATA[0:-N], trace=True, error_action='ignore', suppress_warnings=True,random_state=np.random.randint(low=1, high=100, size=1)[0])
        model.params()
        model.arparams()
        

        O=model.order
        model5 = ARIMA(DATA[0:-N].values,order=O)
        import random
        ini_params=[random.uniform(0,0.6) for i in range(6)]
        
        try:
         model6=model5.fit(disp=0,start_params=ini_params)
        except:
            print('SVD Error')
        model6.params
        ...
        for parameters in range(100):
            O=model.order
            model5 = ARIMA(DATA[0:-N].values,order=O)
            import random
            ini_params=[random.uniform(0,0.3) for i in range(6)]
            
            try:
             model6=model5.fit(disp=0,start_params=ini_params)
             param_list.append(model6.params)
            except:
                print('SVD Error')
        ...
        if P==0:
         ...
         def prediction_interval(y,yhat):
            ...
            return [lower,upper]
         #prediction_interval(Yoriginal,YHAT)a
          
         #For Next Predictiom
         PI=prediction_interval(actual_values,predicted_values)
         
         print("Prediction Interval for next prediction(with 95% confidence) :",PI)
         
         #General Prediction Interval
         
         predicted_values
         smean=np.mean(predicted_values)
         from statistics import variance
         svar=variance(predicted_values)
         sstd=sqrt(svar)
         
         PIgl=smean-1.96*sstd
         PIgu=smean+1.96*sstd

         print('Generalized Prediction Interval:% 5.5f &  % 5.2f'%(PIgl,PIgu))         
        ...
        if P!=0:
            ...
            print(lstm_model.layers[0].trainable_weights)
            
            #Prediction Interval
            x=test_scaled[0:, 0:-1]
            YHAT=[]
            for nums in range(len(x)):
              Ypred= lstm_model.predict(x[nums].reshape(1,x[nums].shape[0],1))
              lstm_model.reset_states()
              X=x[nums]
              #X=X.reshape(X.shape[0],1)
              Unite=np.append(X,Ypred[0],axis=0)
              Unite=Unite.reshape(1,lag+1)
              rescaled_unite=scaler.inverse_transform(Unite)
              YHAT.append(rescaled_unite[0][2])
           ...
           prediction_interval(actual_values,predicted_values)
