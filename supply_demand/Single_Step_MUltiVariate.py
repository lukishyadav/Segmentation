...
def fit_lstm(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons):
	...
	for i in range(nb_epoch):
		model.fit(X, y, epochs=1,validation_split = 0.1, batch_size=n_batch, verbose=1, shuffle=False)
		model.reset_states()
	return model 
...
def fit_lstm2(train, n_lag, n_seq, n_batch, nb_epoch, n_neurons):
    ...
    #h=model.fit(X, y, epochs=nb_epoch,validation_split=0.1, batch_size=n_batch, verbose=1, shuffle=False)
	return model ,h 
...
"""
#series=series_to_supervised(DF['counts'].to_frame(), n_in=1, n_out=1, dropnan=False)
...
Ty=test[:,2].reshape(-1,1)
Ty=Ty.reshape(Ty.shape[0], 1, Ty.shape[1])


model,h=fit_lstm2(train, n_lag, n_seq, n_batch, n_epochs, n_neurons)
...
filename = '/Users/lukishyadav/Desktop/Segmentation/supply_demand/finalized_model.sav'
joblib.dump({"model":model,"history":h},filename)
...
#h=model.fit(X, y,validation_split = 0.1, epochs=1, batch_size=n_batch, verbose=1, shuffle=False)
