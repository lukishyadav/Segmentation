...
def fit_lstm5(train,batch_size,nb_epoch,neurons,act,sf=False,pat=100,lss='mean_squared_error',opt='adam',met=['mse'],monit='val_loss'):
    mc = ModelCheckpoint('best_model.h5', monitor=monit, mode='min', save_best_only=True,verbose=1)
    es = EarlyStopping(monitor=monit, mode='min', verbose=1, patience=pat)
    X,y=train[:,0:-1],train[:,-1]
    X=X.reshape(X.shape[0],X.shape[1],1)
    model=Sequential()
    #model.add(LSTM(neurons, batch_input_shape=(batch_size, X.shape[1], X.shape[2]), stateful=True))
...
# fit the model
#lstm_model,h,es = fit_lstm2(train_scaled, 1, 1000, 50)

#lstm_model,h,es = fit_lstm4(train_scaled, 1, 1000, 100,'tanh')

# 'val_loss', 'val_mean_squared_error', 'loss', 'mean_squared_error'
lstm_model,h,es = fit_lstm5(train_scaled, 1, 1000, 100,'tanh',
                            False,100,'mean_squared_error','adam',['mse'],'val_mean_squared_error')
