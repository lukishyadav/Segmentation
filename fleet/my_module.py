...
def performance_S(model,X,Y,seed,split):

    kfold = model_selection.StratifiedKFold(n_splits=split, random_state=seed)
    cv_results = model_selection.cross_val_score(model,X,Y, cv=kfold, scoring='recall')
    print('recall: ',cv_results.mean(),cv_results)
    
    kfold = model_selection.StratifiedKFold(n_splits=split, random_state=seed)
    cv_results = model_selection.cross_val_score(model,X,Y, cv=kfold, scoring='precision')
    print('precision: ',cv_results.mean(),cv_results)
    
    kfold = model_selection.StratifiedKFold(n_splits=split, random_state=seed)
    cv_results = model_selection.cross_val_score(model,X,Y, cv=kfold, scoring='accuracy')
    print('accuracy: ',cv_results.mean(),cv_results)
    
def performance(model,X,Y,seed,split):
