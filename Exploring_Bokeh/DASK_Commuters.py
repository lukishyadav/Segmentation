...
df['s_lng_col']=df['s_try'].map(lambda x:float(x[1]),meta=pd.Series([], dtype=float, name='ss'))
...
df['e_lat_col']=df['e_try'].map(lambda x:float(x[0]))

df['e_lng_col']=df['e_try'].map(lambda x:float(x[1]))
