...
"""
def convert_to_mercator(X):
    projection = Proj(init='epsg:3857')
    xs = []
    ys = []
    for lng, lat in zip(list(X[0]),list(X[1])):
        x, y = projection(lng, lat)
        xs.append(x)
        ys.append(y)
    return xs, ys
...
#df[['merc_lng','merc_lat']]=df[['start_lng','start_lat']].apply(convert_to_mercator,axis=1)
...
#x, y = projection(37.8761,-122.25939)
"""
...
p = figure(x_range=(minlng, maxlng), y_range=(minlat,maxlat),
           x_axis_type="mercator", y_axis_type="mercator")merc_lng
...
filtered_df=set_clusters(df[df['customer_id']==21],5,5,100)
